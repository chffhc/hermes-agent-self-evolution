...
def build_report(
    ...
    story.append(
        Paragraph(
            ...
            "and observed a <b>+39.5% score increase</b> on one of two held-out validation examples, "
            "measured with a keyword-overlap proxy metric. This is an end-to-end smoke test: it shows "
            "the pipeline runs and can move its local proxy metric. With n=2 holdout examples and a "
            "heuristic scorer, it is <b>not</b> statistically meaningful evidence of improved skill "
    ...
    # Key result box
    result_data = [
        ["KEY RESULT (proxy metric, single held-out example)"],
    ...
    story.append(
        Paragraph(
            "The optimized skill showed a <b>+39.5% proxy-score increase</b> on the first validation "
            "example and no change on the second, for a <b>+20.7%</b> average across the two. This was "
            "achieved with the simplest DSPy optimizer (BootstrapFewShot) using only 3 training "
            "examples and a single optimization round completing in under 60 seconds. With two "
            "held-out examples and a keyword-overlap heuristic, these numbers demonstrate pipeline "
            "viability — they do not establish a validated quality gain, which requires larger "
            "evaluation sets and benchmark gating (see Roadmap).",
            styles["BodyJust"],
        )
    )

    story.append(Paragraph("How the Improvement Was Achieved", styles["SubSection"]))
    story.append(Paragraph("DSPy's BootstrapFewShot optimizer works by:", styles["BodyJust"]))
    improve_steps = [
    ...
    roadmap_data = [
        ...
        ["Phase 1", "Skill files (SKILL.md)", "DSPy + GEPA", "3-4 weeks", "Smoke-tested ✓"],
    ...
    roadmap_table = Table(
        roadmap_data, colWidths=[0.9 * inch, 1.6 * inch, 1.3 * inch, 1.0 * inch, 1.0 * inch]
    )
