...
class TestConfigIntegration:
    ...
    def test_make_lm_returns_dspy_lm(self):
        ...
        with (
            patch("evolution.core.config.get_api_key", return_value="sk-test-key"),
...
class TestPromptEvolutionIntegration:
    ...
    def test_validate_sections_ok(self):
        ...
        sections = [
            PromptSection(
                name="TEST",
                content="some guidance with helpful direct honest keywords",
                file_path="f.py",
                description="d",
                max_growth_pct=50,
                risk_level="low",
    ...
    def test_prompt_section_fitness_metric(self):
        ...
        example = dspy.Example(expected_behavior="search memory for past context").with_inputs(
            "scenario"
        )
...
class TestOptimizerWiring:
    ...
    def test_gepa_importable(self):
        ...
        assert hasattr(dspy, "GEPA") or hasattr(
            dspy, "MIPROv2"
    ...
    def test_miprov2_importable(self):
        ...
        assert hasattr(dspy, "MIPROv2"), "MIPROv2 not found in DSPy — check installation"
